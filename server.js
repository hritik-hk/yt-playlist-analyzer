...
app.post('/get-length', getPlaylistData)
...
async function getPlaylistData(req,res){
    ...
    if(playlistID===''){ //checking if the link is valid or not

    //   errorDiv.innerHTML=`Invalid Link... "list" parameter not found !`
    //   loader.classList.add("hide");
    //   errorDiv.classList.remove("hide");

      const data={ok:false, error: `Invalid Link... "list" parameter not found !`}
 ...
 while(true){
    ...
    video_list1.push(...playlistContent.items.map(item=>item.contentDetails.videoId)); 
    ...
    //    errorDiv.innerHTML=`Invalid Link... Make sure you paste the correct link !`
        data={ok:false, error: `Invalid Link... Make sure you paste the correct link !`}
      }else{
...
// max limit is 50 video IDs
 ...
 const URL2=`https://www.googleapis.com/youtube/v3/videos?&part=contentDetails&id=${videoIds}&key=${apiKey}&fields=items/id,items/contentDetails/duration`;
    ...
    try{
      ...
      vid_iso_duration.push(...data.items.map(item => item.contentDetails.duration));
